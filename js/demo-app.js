...
const app = {
    ...
    setupAssessmentForm() {
        ...
        const selects = ['assess-income', 'assess-employment', 'assess-emi', 'assess-purpose'];
    ...
    assessAndBucket() {
        ...
        const loanPurpose = document.getElementById('assess-purpose').value;
        const userName = document.getElementById('assess-name').value.trim();

        // ── Bucketing Algorithm ──
        // Score each dimension, then classify
        // Credit history is NOT collected here — it is fetched from the Credit Bureau later
        let score = 0;
        ...
        // EMI burden scoring (max 10, lower burden is better)
        ...
        // Total possible: 25. A ≥ 18, B ≥ 12, C < 12
        // Hard rule: extreme debt burden auto-declines
        let bucketId;
        if (foirEstimate > 55) {
            bucketId = 'C'; // Hard rule: very high existing debt burden
        } else if (score >= 18) {
